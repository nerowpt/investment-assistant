...
> **MVP-1 启用集**（2026-05-22 Review，见 [docs/06 §D9](06-架构Review决议.md)）：
> `media_type ∈ {text, pdf, html, structured}`。
> image / audio / video 字段已预留，**MVP-1 写入将被 `inv doctor --scope library` warning**（不 fatal），
> 避免将本属 MVP-3+ 的能力误用为 MVP-1 主路径。
...
| FTS | ❌ → **H2 评估** | **不晚于 H8 之前**（见 [docs/06 §D10](06-架构Review决议.md)） |

> **FTS5 评估前置（D10）**：MVP-1 H2 末期执行一次"FTS5 接入难度评估"：
> - 通过门槛：modernc.org/sqlite 原生支持 FTS5 + 接入工作量 ≤ 1 周。
> - 通过 → FTS5 **提前到 H2 完成**（MVP-1 范围内），避免 library_items > 500 时检索体验崩塌。
> - 不通过 → FTS5 最迟 **H8 之前**（MVP-2A 起点）必须完成。
> - MVP-1 即使无 FTS5，`inv library search` 也必须支持：title LIKE + tags 过滤 + related_stocks 精确匹配 + tier 过滤 的组合查询。
| 10A.1 表（含 `swap` 占位） | 见 §10A.1 | `swap` 仅占位，MVP-2A 才实现（[docs/06 §D7](06-架构Review决议.md)） |
| `lots` 复权与分红字段 | 字段位预留，默认值 | MVP-2 H13 填充（[docs/06 §D8](06-架构Review决议.md)） |
...
| **`swap`**（MVP-2A 起） | + 2 条 `journals`（sell + buy）共享 `swap_pair_id` | `portfolio.yaml`（A 减、B 增） | ✅ × 2 | ✅ B 端新 lot + A 端关闭/部分关闭 |
...
| `created_at` | TEXT | 是 | |
| `dividends_received` | REAL | 否 | **逻辑 DECIMAL**。已收现金分红累计（元）。MVP-1 默认 0；用户可手填；MVP-2 H13 从 akshare 自动拉。**见 [docs/06 §D8](06-架构Review决议.md)** |
| `adjusted_cost_basis` | REAL | 否 | **逻辑 DECIMAL**。前复权调整后成本价。MVP-1 默认 NULL（= 用 `cost_basis`）；MVP-2 计算 |
| `corporate_actions_json` | TEXT | 否 | **逻辑 JSON**。送转 / 拆股 / 配股事件流水数组，每项 `{date, type, ratio, notes}`。MVP-1 默认空数组 |
