...
| 任务 | 触发 | 实现 |
|---|---|---|
| `library_crawl` | daily 08:00（`data_sources.yaml`） | scheduler → app → worker.FetchAnnouncements → StageCandidate |
| `library expire` | weekly | status → expired |
| `backup warn` | checklist 提交前 | app 读 backup 元数据 |

MVP-1：`inv serve` 内嵌 **robfig/cron**；无 serve 时需用户手动 `inv library crawl`。
